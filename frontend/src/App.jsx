...
export default function App() {
  ...
  async function runSearch(value, more = false) {
    ...
    try {
      ...
      setMessage(data.items.length || more ? '' : `No results for “${cleaned}”. Try an exact gene symbol such as BRCA1 or a numeric VariationID.`);
  ...
  return <>
    ...
    <main>
      ...
      <section className="search-panel" id="explorer"><div><p className="eyebrow">EXPLORE THE DATA</p><h2>Start with a gene or variant.</h2></div>
        ...
        <div className="examples"><span>TRY AN EXAMPLE</span>{['BRCA1', 'TP53', '17660'].map(value => <button key={value} onClick={() => runSearch(value)}>{value === '17660' ? 'Variant 17660' : value}</button>)}<small>Exact gene symbols · Numeric variant IDs</small></div>
      </section>
