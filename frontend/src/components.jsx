...
export function Badge({ state, label }) {
  return <span className={`badge ${Object.hasOwn(stateLabels, state) ? state : ''}`}>{label || stateLabels[state] || 'Unavailable'}</span>;
}
...
export function Results({ result, loading, onMore, onSelect }) {
  ...
  return <section className="panel" aria-label="Search results">
    ...
    <div className="table-scroll"><table><thead><tr><th>VARIANT ID</th><th>GENE</th><th>VARIANT NAME</th><th>RECORDED CLASSIFICATION</th><th>PREDICTED CLASS · RESEARCH</th><th><span className="sr-only">Action</span></th></tr></thead>
      <tbody>{result.items.map(item => <tr key={item.variation_id}>
        ...
        <td><Badge state={item.latest_details?.clinical_state || item.timeline?.at(-1)?.state} label={item.latest_details?.clinical_significance} /></td>
        <td>{item.prediction?.status==='available'?<><strong>{item.prediction.predicted_class}</strong><small className="prediction-table-percent">{percentage(item.prediction.top_probability)} model estimate</small></>:item.prediction?.reason||'Select to calculate'}</td>
...
export function Detail({ selected, onClose, onRetry }) {
  ...
  return <section className="panel details" ref={sectionRef} aria-label="Selected variant">
    ...
    {selected.data && <div className="detail-content">
      ...
      <PredictionSummary prediction={selected.data.prediction} />
      <details className="historical-details"><summary>Observed classification history</summary><p className="footnote">Historical evidence is separate from the prediction above.</p>
      <div className="timeline" aria-label="Six snapshot observations">{selected.data.timeline.map(observation =>
        <div className="timeline-item" key={observation.snapshot}><time dateTime={observation.snapshot}>{observation.snapshot}</time><Badge state={observation.state} label={observation.snapshot===details?.snapshot?details.clinical_significance:undefined} /></div>
      )}</div>
      <p className="footnote">Older cached timelines retain grouped states; the latest label above is preserved exactly. A pathogenic or benign group includes its likely category. Missing GRCh38 observations do not establish removal from ClinVar.</p></details>
    </div>}
  </section>;
}
